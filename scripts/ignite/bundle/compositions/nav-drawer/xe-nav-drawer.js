...
function handleFocusTrap(e7, root) {
  ...
  const active = e7.composedPath().find((n5) => focusable.includes(n5));
...
var XENavDrawer = class XENavDrawer2 extends i4 {
  ...
  _onSelectorSlotChange(e7) {
    const slot = e7.target;
    this._hasSelector = slot.assignedNodes({ flatten: true }).length > 0;
  }
  _handleExpand(e7) {
    const { label, children: detailChildren } = e7.detail;
    const sourceItem = e7.target;
    let children;
