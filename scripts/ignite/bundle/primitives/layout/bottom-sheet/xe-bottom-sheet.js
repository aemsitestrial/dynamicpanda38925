...
var n4 = (t5) => (r6, ...e7) => {
  ...
  let c5, $3 = 0, f3 = false;
  for (; $3 < a4; ) {
    for (c5 = r6[$3]; $3 < a4 && void 0 !== (i7 = e7[$3], s4 = o5(i7)); ) c5 += s4 + r6[++$3], f3 = true;
...
function handleFocusTrap(e7, root) {
  if (e7.key !== "Tab")
    return;
  ...
  const active = e7.composedPath().find((n6) => focusable.includes(n6));
  if (e7.shiftKey && (active === first || !active)) {
    e7.preventDefault();
    last.focus();
  } else if (!e7.shiftKey && (active === last || !active)) {
    e7.preventDefault();
    first.focus();
  }
}
...
var XEBottomSheet = class XEBottomSheet2 extends i4 {
  constructor() {
    super(...arguments);
    this.open = false;
    this.variant = "modal";
    ...
    this._handleKeyDown = (e7) => {
      if (e7.key === "Escape" && this.dismissible) {
        this._close();
        return;
      }
      if (this.variant !== "persistent" && this.shadowRoot)
        handleFocusTrap(e7, this.shadowRoot);
    };
  }
  disconnectedCallback() {
  ...
  get _titleTag() {
    ...
    return level === 1 ? i5`h1` : level === 2 ? i5`h2` : level === 3 ? i5`h3` : level === 4 ? i5`h4` : level === 5 ? i5`h5` : i5`h6`;
  }
  _onActionsSlotChange(e7) {
  ...
  render() {
    if (!this.open && !this._closing)
      return A;
    return b2`
      ${this.variant !== "persistent" ? b2`<div class="overlay ${this._closing ? "closing" : ""}" @pointerdown=${this.dismissible ? this._close : A}></div>` : A}
      <div
      ...
      >
        ${this.title || this.subtitle ? u3`
          <div class="header">
            <div class="header-content">
              ${this.title ? u3`<${this._titleTag} class="title">${this.title}</${this._titleTag}>` : A}
              ${this.subtitle ? b2`<p class="subtitle">${this.subtitle}</p>` : A}
            </div>
            ${this.dismissible ? b2`
...
XEBottomSheet.styles = xe_bottom_sheet_default;
__decorate3([
  n5({ type: Boolean, reflect: true })
...
__decorate3([
  n5({ type: Number, attribute: "title-heading-level" })
