...
var XEButton = class XEButton2 extends i4 {
  constructor() {
    super(...arguments);
  ...
  render() {
    ...
    const commonContent = b2`
      <span class="content">
        ${this.leadingIcon ? b2`<span class="icon"><xe-icon icon=${this.leadingIcon} size="sm"></xe-icon></span>` : A}
        <slot name="icon"></slot>
        <span class="text">
          <slot></slot>
        </span>
        <slot name="trailing-icon"></slot>
        ${this.trailingIcon ? b2`<span class="trailing-icon"><xe-icon icon=${this.trailingIcon} size="sm"></xe-icon></span>` : A}
      </span>
    `;
    if (this.href && !this.disabled) {
      return b2`
        <a
          ...
          aria-haspopup=${this._ariaHaspopup || A}
          aria-expanded=${this._ariaExpanded || A}
...
XEButton.styles = xe_button_default;
__decorate2([
  n4({ type: String })
...
__decorate2([
  n4({ type: String, attribute: "size-mobile" })
...
var XEMenuItem = XEMenuItem_1 = class XEMenuItem2 extends i4 {
  ...
  _handleKeyDown(e7) {
    ...
    } else if (e7.key === "Escape" && this._submenuOpen) {
      e7.stopPropagation();
      this._submenuOpen = false;
      if (XEMenuItem_1._openMenuItem === this)
        XEMenuItem_1._openMenuItem = null;
    } else if (e7.key === "ArrowDown" || e7.key === "ArrowUp") {
      e7.preventDefault();
      e7.stopPropagation();
      this._moveSiblingFocus(e7.key === "ArrowDown" ? 1 : -1, e7.composedPath());
    } else if (e7.key === "ArrowRight" && this._hasSubmenu) {
      e7.preventDefault();
      e7.stopPropagation();
...
XEMenuItem = XEMenuItem_1 = __decorate4([
  t3("xe-menu-item")
], XEMenuItem);
...
var XEMenuButton = class XEMenuButton2 extends i4 {
  constructor() {
    super(...arguments);
    this.trigger = "button";
    this.label = "";
    ...
    this._open = false;
    this._handleFocusOut = (e7) => {
      const related = e7.relatedTarget;
      if (!related || !this._containsNode(related)) {
        this._closeMenu();
      }
    };
    this._handleKeyDown = (e7) => {
      if (e7.key === "Escape" && this._open) {
        e7.stopPropagation();
        this._closeMenu(true);
      } else if ((e7.key === "ArrowDown" || e7.key === "ArrowUp") && this._open) {
        e7.preventDefault();
        e7.stopPropagation();
        this._moveFocus(e7.key === "ArrowDown" ? 1 : -1, e7.composedPath());
      } else if (e7.key === "Tab" && this._open) {
        e7.preventDefault();
        ...
        const direction = e7.shiftKey ? -1 : 1;
  ...
  _moveFocus(direction, path) {
    ...
    if (!items.length)
      return;
    const activeIndex = items.findIndex((item) => path.includes(item));
    const next = activeIndex === -1 ? direction === 1 ? 0 : items.length - 1 : activeIndex + direction;
  ...
  _renderTrigger() {
    ...
    return b2`
      <xe-button
        ...
        aria-haspopup="true"
        @click="${this._toggleMenu}">
        ${this.icon ? b2`<xe-icon slot="icon" icon="${this.icon}" size="sm"></xe-icon>` : ""}
        ${this.label}
      </xe-button>
    `;
  }
  render() {
    return b2`
      ${this._open ? b2`<div class="backdrop" @pointerdown="${() => this._closeMenu()}"></div>` : A}
      <div class="menu-button" @focusout=${this._handleFocusOut} @keydown=${this._handleKeyDown}>
        ...
        ${this._open ? b2`
          <div class="menu ${this.menuAlign === "end" ? "align-end" : ""}"
               @click="${(e7) => {
      if (e7.composedPath().some((el) => el instanceof Element && el.tagName.toLowerCase() === "xe-menu-item")) {
        this._closeMenu(true);
      }
    }}">
...
XEMenuButton.styles = [xe_menu_button_default];
__decorate6([
  n4({ type: String })
...
__decorate6([
  n4({ type: String, attribute: "menu-variant" })
], XEMenuButton.prototype, "menuVariant", void 0);
__decorate6([
  n4({ type: String, attribute: "menu-align" })
], XEMenuButton.prototype, "menuAlign", void 0);
__decorate6([
  n4({ type: String, attribute: "menu-width" })
